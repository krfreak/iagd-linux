// Fork of iagd/Parser/Arc/DDSImageReader.cs.
//
// Upstream needs System.Drawing, which throws TypeInitializationException (GDI+
// unavailable) on Linux since .NET 7. Of 942 lines only 18 touched System.Drawing; the
// rest is DDS/DXT decoding, a format that does not change. So this file is forked and the
// drawing calls replaced with PngEncoder, while the volatile ARC/ARZ parsing next to it
// stays a reference to upstream.
//
// Changes from upstream, all confined to image output:
//   * ExtractImage returns DecodedImage (width/height/ARGB pixels) instead of Image
//   * BitmapFromBytes removed - nothing wraps the pixels any more
//   * Bitmap.Save  -> PngEncoder.WriteArgb
//   * already-PNG entries are copied verbatim rather than decoded and re-encoded, which
//     is both faster and lossless
using System;
using log4net;
using System.IO;
...
// Parser has its own internal IOHelper, invisible from this assembly. EvilsoftCommons
// exposes a public one carrying the two members this file needs (GetInt/SetInt), so alias
// to that rather than copying code or widening upstream's visibility.
using IOHelper = EvilsoftCommons.IOHelper;

namespace IAGrim.Core.Imaging {
    /// <summary>Decoded image data: one packed 0xAARRGGBB value per pixel, row-major.</summary>
    public sealed record DecodedImage(int Width, int Height, int[] Pixels);

    public class DdsIconExtractor {
                ...
                if (!File.Exists(itemsArcFullPath)) {
                    ...
                    foreach (var icon in dc.strings) {
                        ...
                        if (icon.EndsWith(".png")) {
                            ...
                            try {
                                // Upstream decoded these through GDI+ purely to read the
                                // dimensions, then re-encoded. The size lives in the IHDR
                                // header, so read it directly and copy the file through
                                // untouched - faster, and lossless.
                                if (!PngHeader.TryReadSize(b, out var w, out var h)) {
                                    logger.Warn($"Skipping {icon}, not a readable PNG");
                                    continue;
                                }
                            ...
                            try {
                                ...
                                // Upstream disposed each Bitmap here: they held unmanaged GDI+
                                // memory that the GC under-counted, which caused out-of-memory
                                // during icon extraction on lower-end machines. DecodedImage is
                                // a plain managed array, so that hazard is gone with GDI+.
                                var img = ExtractImage(b);
                                if (img != null) {
                                    PngEncoder.WriteArgb(imagePath, img.Width, img.Height, img.Pixels);
                                }
                            }
                            catch (Exception ex) {
                                logger.Warn($"Error extracting icon \"{icon}\", {ex.Message}", ex);
        ...
        public static DecodedImage? ExtractImage(byte[] bytes) {
            ...
            return new DecodedImage(GetWidth(rawPixels), GetHeight(rawPixels), decodedPixels);
        }

        // BitmapFromBytes (both overloads) removed: ExtractImage now returns raw pixels.
